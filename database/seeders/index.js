...
async function main() {
    ...
    const createrole = 'CREATE TABLE `depttracker_db`.`role` (`id` INT NOT NULL AUTO_INCREMENT,`title` VARCHAR(30) NOT NULL,`salary` VARCHAR(45) NOT NULL,`department_id` INT NULL,PRIMARY KEY (`id`),INDEX `department_id_idx` (`department_id` ASC) VISIBLE,CONSTRAINT `department_id`FOREIGN KEY(`department_id`) REFERENCES`depttracker_db`.`department`(`id`) ON DELETE SET NULL ON UPDATE NO ACTION); ';
    await db.execute(createrole);
    await seedRoles();

    /* await db.execute('DROP TABLE IF EXISTS role');
    const createemployee='CREATE TABLE `depttracker_db`.`department` (`id` INT NOT NULL AUTO_INCREMENT,`name` VARCHAR(30) NOT NULL,PRIMARY KEY (`id`))';
